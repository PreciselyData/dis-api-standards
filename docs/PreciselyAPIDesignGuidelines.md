...
below are based on industry wide practices, pragmatic REST principles
...
API Versions are represented as V1, V2, etc. Any minor version must be
backwards compatible and therefore does not require a change in version
number.
...
**Developers must perform resource modeling as part of the API design
process.**
...
HTTP method should define the action to be performed.**
...
applications being forced to invoke multitude of APIs to accomplish a
...
Based on [W3
specification](https://www.w3.org/TR/WD-html40-970708/htmlweb.html),
...
<tr class="odd">
<td>GET</td>
<td><p>On a collection</p>
<p>/products</p></td>
<td>Return a list of products. It should return a 200 response with an
empty array (or array within a pagination type) when there are no
resources to return.</td>
</tr>
<tr class="even">
<td>GET</td>
<td><p>On a resource</p>
<p>/products/111</p></td>
<td>Get details on products with id 111. Additional parameters required
to perform the GET operation are passed as query parameters.</td>
</tr>
<tr class="odd">
<td>POST</td>
<td><p>On a collection</p>
<p>/products</p></td>
<td>Create a new product. The details of the record to be created is
provided as part of the POST body. Response should return an HTTP
Location header that is the GET URL of the resource.</td>
</tr>
<tr class="even">
<td>PUT</td>
<td><p>On a resource</p>
<p>/products/111</p></td>
<td>Update the entire record 111 with the contents of the body.</td>
</tr>
<tr class="odd">
<td>PATCH</td>
<td><p>On a resource</p>
<p>/products/111</p></td>
<td>Updates the record 111 only with the fields present in the body.
(i.e. partial updates)</td>
</tr>
<tr class="even">
<td>DELETE</td>
<td><p>On a resource</p>
<p>/products/111</p></td>
<td>Deletes record 111. May also operate on a collection.</td>
</tr>
<tr class="odd">
<td>OPTIONS</td>
<td>On a resource or collection</td>
<td>Returns a list of supported HTTP methods</td>
</tr>
</tbody>
</table>

> Note regarding POST for getting information on resources.
>
...
| 503 SERVICE UNAVAILABLE    | The server is currently unable to handle the request due to a temporary overload or scheduled maintenance, which will likely be alleviated after some delay. The server MAY send a Retry-After header field[<sup>1</sup>](https://httpstatuses.com/503#ref-1) to suggest an appropriate amount of time for the client to wait before retrying the request. |
...
<p>429 TOO MANY REQUESTS</p></td>
<td>Optional</td>
</tr>
<tr class="odd">
<td><p>500 INTERNAL SERVER ERROR</p>
<p>503 SERVICE UNAVAILABLE</p></td>
<td>Error body that provides an error code that will help debug the
server issue. Any other fields included in the error response MUST NOT
provide any internal secure information that is private to
Precisely.</td>
</tr>
</tbody>
</table>

The format for the error body should follow [RFC
7807](https://tools.ietf.org/html/rfc7807) “Problem Details”, an
...
JSON as the response body content type duly including the Content-Type
header.
...
interactions and current context information (e.g., cookie etc.,).
...
the server whenever it is needed. The APIs should not create any session
...
An idempotent HTTP request is a request that can be invoked multiple
...
resource already exists, and the client will be forced to invoke a
...
API security should be an integral part of the API design. API Gateway
...
APIs published should have Open API Specification for API documentation.
The Open API Specification contract describes what the API does, it’s
request parameters and response objects, all without any indication of
code implementation. Web services defined with Open API Specification
...
Open API Specs Generation During Build time: Every API should
incorporate a mechanism of to generate the Open API Specification based
on the meta-data, such as code annotations, applied to various
resources, methods, and controllers. This meta-data will generate the
...
servlets, using these libraries in java you can generate the Open API
